...
contract Address{
    ...
    function enrollStudentInClass(
        string memory _class, uint _id, string memory _name,
            bool _hasDegree, Gender _gender, ClassType _classtype) public payable {

        require(_hasDegree == true, "You must have BS degree");
        require(msg.value == 2 ether, "You must pay 2 Ether for Registration");

        Class[_class][_id] = StudentStruct({
            name: _name, 
    ...
    function getEnrolledStudent(uint _id, string memory _class) public view returns (
        string memory, address, bool, Gender, ClassType)
    {
        
        return (
            Class[_class][_id].name,
            Class[_class][_id].ethAddress,
            Class[_class][_id].hasDegree,
            Class[_class][_id].gender,
            Class[_class][_id].classtype
            );
    }
